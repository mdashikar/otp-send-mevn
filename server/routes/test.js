...
const OTP = require('../models/test');
const accountSid = 'ACe9f4ae4536388c6a43e16e8396c6e907';
const authToken = 'cba290877941edadebc5639d1e2ad15b';
...
router.get('/get-otp-users', (req, res) => {
    ...
    //   res.json(users);
    // });
    OTP.find({}).sort({updated: -1}).exec(function(err, users) {
      res.json(users);
    });
});
    ...
    myData.save()
        .then(item => {
            client.messages.create({
                to: '+8801750358866',
                from: '+14253826838',
                body: `Hi, Your one time pass is ${req.body.otpNumber}.` 
            }).then( (message) => {
